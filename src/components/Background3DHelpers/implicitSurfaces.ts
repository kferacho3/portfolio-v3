      ...
      for (let k = 0; k < resolution; k++) {
        ...
        if (edgeVerts.length >= 3) {
          ...
          for (let e = 0; e < edgeVerts.length; e++) {
            ...
            const eps = step * 0.1;
            const nx =
              (fn(centroid[0] + eps, centroid[1], centroid[2]) -
                fn(centroid[0] - eps, centroid[1], centroid[2])) /
              (2 * eps);
            const ny =
              (fn(centroid[0], centroid[1] + eps, centroid[2]) -
                fn(centroid[0], centroid[1] - eps, centroid[2])) /
              (2 * eps);
            const nz =
              (fn(centroid[0], centroid[1], centroid[2] + eps) -
                fn(centroid[0], centroid[1], centroid[2] - eps)) /
              (2 * eps);
...
function createIsoSurfaceGeometry(
  ...
  const { resolution = 32, bounds = 2, isoValue = 0, scale = 1 } = params;
 ...
 */
export function gyroidSurfaceGeometry(
  params: IsoSurfaceParams = {}
): THREE.BufferGeometry {
 ...
 */
export function schwarzDSurfaceGeometry(
  params: IsoSurfaceParams = {}
): THREE.BufferGeometry {
  const fn: ImplicitFunction = (x, y, z) => {
    const scale = Math.PI;
 ...
 */
export function schwarzPSurfaceGeometry(
  params: IsoSurfaceParams = {}
): THREE.BufferGeometry {
  const fn: ImplicitFunction = (x, y, z) => {
    const scale = Math.PI;
 ...
 */
export function neoviusSurfaceGeometry(
  params: IsoSurfaceParams = {}
): THREE.BufferGeometry {
  const fn: ImplicitFunction = (x, y, z) => {
    const scale = Math.PI;
    const sx = x * scale,
      ...
      sz = z * scale;
    return (
      3 * (Math.cos(sx) + Math.cos(sy) + Math.cos(sz)) +
 ...
 */
export function lidinoidSurfaceGeometry(
  params: IsoSurfaceParams = {}
): THREE.BufferGeometry {
  const fn: ImplicitFunction = (x, y, z) => {
    const scale = Math.PI;
 ...
 */
export function genus2SurfaceGeometry(
  params: IsoSurfaceParams = {}
): THREE.BufferGeometry {
  const fn: ImplicitFunction = (x, y, z) => {
    const a = 0.6;
    ...
    const torus1 =
      Math.pow(x2 + y2 + z2 + a * a - 0.1, 2) - 4 * a * a * (x2 + y2);
    const torus2 =
      Math.pow(x2 + y2 + z2 + a * a - 0.1, 2) - 4 * a * a * (y2 + z2);
...
export function chmutovSurfaceGeometry(
  ...
  const fn: ImplicitFunction = (x, y, z) => {
    ...
    const cheb = (t: number, n: number): number =>
      Math.cos(n * Math.acos(Math.max(-1, Math.min(1, t))));
