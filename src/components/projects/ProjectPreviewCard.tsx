 ...
 *  Glass "browser window" preview. Desktop: floats beside the node.
 *  Touch / narrow: docks as a bottom sheet so it never fights the map.
 * ===================================================================== */
'use client';
    ...
    <>
      ...
      <div
        className={`relative overflow-hidden bg-black ${
          compact ? 'aspect-[16/9] max-h-36' : 'aspect-[16/10]'
        }`}
      >
        ...
        {logoSrc && (
          <div
            className="absolute bottom-3 left-3 grid h-10 w-10 place-items-center overflow-hidden rounded-full border bg-black/70"
            style={{ borderColor: `${node.accent}99` }}
          >
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={logoSrc}
      ...
      <div className={`space-y-3 ${compact ? 'p-3.5' : 'p-4'}`}>
        <div>
          ...
          <h4 className="mt-0.5 text-base font-bold text-white">{node.title}</h4>
        </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {node.project.techStack.slice(0, compact ? 3 : 4).map((t) => (
            <span
              key={t}
        ...
        <div className="flex gap-2 pt-1">
          <button
            type="button"
            onClick={() => onOpen(node)}
            className="min-h-11 flex-1 rounded-xl px-3 py-2.5 text-center text-xs font-semibold text-white transition"
          ...
          <a
            ...
            className="min-h-11 flex-1 rounded-xl border border-white/15 bg-white/5 px-3 py-2.5 text-center text-xs font-semibold text-white transition hover:bg-white/10"
          >
            Visit Live
          </a>
        </div>
...
export default function ProjectPreviewCard({
  ...
  if (variant === 'sheet') {
    return (
      <motion.div
        className="pointer-events-auto absolute inset-x-0 bottom-0 z-40 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
      ...
      >
        <div
          className="mx-auto max-w-lg overflow-hidden rounded-2xl border bg-[#0b0a14]/96 backdrop-blur-xl"
          style={{
            borderColor: `${node.accent}55`,
            boxShadow: `0 -12px 40px rgba(0,0,0,0.45), 0 0 28px ${node.accent}22`,
          }}
        >
          <div className="flex justify-center pt-2">
            <span className="h-1 w-10 rounded-full bg-white/20" />
          </div>
          <PreviewBody node={node} compact onOpen={onOpen} />
        </div>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <PreviewBody node={node} onOpen={onOpen} />
      </div>
    </motion.div>
  );
}
