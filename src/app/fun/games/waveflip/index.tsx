...
const waveSample = (x: number, runtime: Runtime) => {
  ...
  const dyDx =
    runtime.waveSign * runtime.waveAmp * runtime.waveFreq * Math.cos(arg);
...
const spawnObstacleSet = (runtime: Runtime) => {
  ...
  const doubleChance =
    runtime.elapsed < 12
      ? 0
      : clamp(0.05 + Math.max(0, tier - 1) * 0.06 + d * 0.12, 0, 0.42);
  const doublePattern = Math.random() < doubleChance;
  const count = doublePattern ? 2 : 1;
...
function WaveFlipScene() {
  ...
  useFrame((_, delta) => {
    ...
    if (store.status === 'PLAYING') {
      ...
      const playerSample = waveSample(PLAYER_X, runtime);
      runtime.playerY = playerSample.yRider;
      runtime.playerNormalY = playerSample.normalY;
    ...
    if (obstacleRef.current) {
      for (let i = 0; i < runtime.obstacles.length; i += 1) {
        ...
        colorScratch
          .copy(obstacle.requiredSign === 1 ? SPIKE_BOTTOM : SPIKE_TOP)
          .lerp(WHITE, clamp(obstacle.glow * 0.5, 0, 0.65));
  ...
  return (
    <>
      ...
      <EffectComposer enableNormalPass={false} multisampling={0}>
        <Bloom intensity={0.48} luminanceThreshold={0.54} luminanceSmoothing={0.22} mipmapBlur />
