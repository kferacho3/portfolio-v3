...
const Walls: React.FC = () => {
  ...
  useFrame(() => {
    ...
    if (leftRef.current) {
      ...
      (leftRef.current.material as THREE.MeshBasicMaterial).color.copy(
        mutation.globalColor
      );
    }
    if (rightRef.current) {
      rightRef.current.position.z = playerZ;
      (rightRef.current.material as THREE.MeshBasicMaterial).color.copy(
        mutation.globalColor
      );
    }
  });
  ...
  return (
    <>
      <mesh
        ref={leftRef}
        position={[LEFT_BOUND, 0, 0]}
        rotation={[Math.PI / 2, 0, 0]}
      >
        <cylinderGeometry
          args={[WALL_RADIUS, WALL_RADIUS, PLANE_SIZE * 2, 8, 1, true]}
        />
        <meshBasicMaterial
          color={COLORS[0].three}
          transparent
          opacity={0.3}
