...
import { calculateFinalPosition, directionToRotation, generateRows, worldXToTile } from './utils/rows';
    ...
    <>
      ...
      {rows.map((rowData, index) => {
        ...
        if (rowData.type === 'road') {
          return <RoadRow key={`row-${rowIndex}`} rowIndex={rowIndex} data={rowData} playerRef={playerRef} onHit={endGame} />;
        }
        if (rowData.type === 'wildlife') {
          return <WildlifeRow key={`row-${rowIndex}`} rowIndex={rowIndex} data={rowData} playerRef={playerRef} onHit={endGame} />;
        }
        if (rowData.type === 'subway') {
          return <SubwayRow key={`row-${rowIndex}`} rowIndex={rowIndex} data={rowData} playerRef={playerRef} onHit={endGame} />;
        }
        if (rowData.type === 'drone') {
          return <DroneLane key={`row-${rowIndex}`} rowIndex={rowIndex} data={rowData} playerRef={playerRef} onHit={endGame} />;
        }
        return (
          <RiverRow
            key={`row-${rowIndex}`}
      ...
      />
      {snap.status === 'over' && (
        <GameOverOverlay score={snap.score} bestScore={snap.bestScore} onRestart={() => fluxHopState.reset()} />
      )}
    </>
  );
};
