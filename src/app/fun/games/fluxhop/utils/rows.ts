...
export const randomChoice = <T,>(values: T[]) => values[Math.floor(Math.random() * values.length)];
...
export const worldXToTile = (x: number) => clamp(Math.round(x / TILE_SIZE), MIN_TILE_INDEX, MAX_TILE_INDEX);
...
export const createVehicleSet = (count: number, sizeOptions: number[], colors: string[], difficulty: number) => {
...
export const createBarrierSet = (count: number, difficulty: number): BarrierData[] => {
...
export const createCritterSet = (count: number, sizeOptions: number[], colors: string[]) => {
...
export const createDroneSet = (count: number, difficulty: number): DroneData[] => {
...
export const generateGrassRow = (rowIndex: number, difficulty: number): GrassRowData => {
...
export const generateRoadRow = (rowIndex: number, difficulty: number): RoadRowData => {
...
export const generateRiverRow = (rowIndex: number, difficulty: number): RiverRowData => {
  const direction = Math.random() > 0.5 ? 1 : -1;
...
export const generateIceRow = (rowIndex: number, difficulty: number): IceRowData => {
  const drift = Math.random() > 0.5 ? 1 : -1;
  const driftSpeed = (0.4 + randomFloat(0, 0.25)) * difficulty;
  return { type: 'ice', drift, driftSpeed };
};

export const generateWildlifeRow = (rowIndex: number, difficulty: number): WildlifeRowData => {
  const direction = Math.random() > 0.5 ? 1 : -1;
...
export const generateSubwayRow = (rowIndex: number, difficulty: number): SubwayRowData => {
  const direction = Math.random() > 0.5 ? 1 : -1;
...
export const generateDroneRow = (rowIndex: number, difficulty: number): DroneRowData => {
...
export const generateRow = (rowIndex: number): RowData => {
  ...
  if (roll < grassWeight + roadWeight + riverWeight + iceWeight + wildlifeWeight + subwayWeight) {
    return generateSubwayRow(rowIndex, difficulty);
  }
  return generateDroneRow(rowIndex, difficulty);
};

export const generateRows = (count: number, startIndex: number, previousType?: RowData['type']) => {
  const rows: RowData[] = [];
...
export const calculateFinalPosition = (current: { rowIndex: number; tileIndex: number }, moves: MoveDirection[]) => {
  return moves.reduce(
    (position, direction) => {
      ...
      if (direction === 'left') return { ...position, tileIndex: position.tileIndex + 1 };
      if (direction === 'right') return { ...position, tileIndex: position.tileIndex - 1 };
      return position;
    },
    { ...current }
  );
