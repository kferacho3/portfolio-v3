...
import { comboIntensity, getMultiBallCount, smashHitState } from './GameStore';
import {
  BALL_PHYSICS,
  GLASS_PHYSICS,
  STONE_PHYSICS,
} from './constants';
import { CameraImpulse } from '../fx/CameraImpulse';
import { updateImpactFlash } from '../fx/ImpactFlash';
import { SmashGlassMaterial } from '../glass/GlassPane';
import GlassShard from '../glass/GlassShard';
import { roomArchetypeFor, themeIndexForRoom } from '../level/LevelGenerator';
import { ROOM_ARCHETYPE_LABEL, type RoomArchetype } from '../level/RoomChunk';
import { emitVolley } from '../player/BallEmitter';
import PostProcessing from '../post/PostProcessing';

export { smashHitState } from './GameStore';
...
function SmashHit() {
  ...
  const world = useRef({
    ...
    cameraY: 0,
    cameraShake: 0,
    cameraImpulse: new CameraImpulse(),
    bloomPunch: 0,
    flashAlpha: 0,
  ...
  const applyCameraImpulse = (
    ...
    w.cameraImpulse.applyImpulse(w.tempVecA, strength);
  };
  ...
  const fireVolley = () => {
    ...
    w.tempVecA.set(camera.position.x, camera.position.y, camera.position.z - 0.55);
    w.projectileCursor = emitVolley({
      projectiles: w.projectiles,
      projectileCursor: w.projectileCursor,
      origin: w.tempVecA,
      pointerX: input.current.pointerX,
      pointerY: input.current.pointerY,
      count,
      shotSpeed: SHOT_SPEED,
      shotTtl: SHOT_TTL,
      cameraVelZ: -w.speed * 0.22,
    });
  ...
  useEffect(() => {
    ...
    const worker = new Worker(
      new URL('../glass/ShardFactory.worker.ts', import.meta.url),
      { type: 'module' }
    );
  ...
  useFrame((_, dtRender) => {
    ...
    const impulseOffset = w.cameraImpulse.update(
      dtRender,
      CAMERA_IMPULSE_DAMPING,
      CAMERA_IMPULSE_STIFFNESS
    );

    camera.position.x = w.cameraX + shakeX + impulseOffset.x;
    camera.position.y = w.cameraY + shakeY + impulseOffset.y;
    camera.position.z = w.cameraZ + impulseOffset.z;
    ...
    smashHitState.activeDebris = countActiveShards();
    smashHitState.setActiveTheme(w.themeTo);
  });
  ...
  return (
    <group>
      ...
      <GlassShard ref={shardsRef} maxShards={MAX_SHARDS} />
      ...
      <PostProcessing
        enabled={tierCfg.bloom}
        bloomEnabled={tierCfg.bloom}
        chromaticEnabled={tierCfg.chromatic}
        noiseEnabled={tierCfg.noise}
        vignetteEnabled={tierCfg.vignette}
        qualityTier={snap.qualityTier}
        bloomIntensity={bloomBaseIntensity}
        chromaticOffset={CHROMATIC_OFFSET}
        bloomRef={bloomRef}
      />
