...
export const smashHitState = proxy({
  ...
  startGame: (seed?: number) => {
    ...
    smashHitState.activeDebris = 0;
    smashHitState.activeTheme = 0;
  ...
  setActiveTheme: (themeIndex: number) => {
    smashHitState.activeTheme = Math.max(0, Math.floor(themeIndex));
  },

  registerDebris: (count: number) => {
    const delta = Math.max(0, Math.floor(count));
    smashHitState.activeDebris = clampInt(
      smashHitState.activeDebris + delta,
      0,
      9999
    );
  },

  releaseDebris: (count: number) => {
    const delta = Math.max(0, Math.floor(count));
    smashHitState.activeDebris = clampInt(
      smashHitState.activeDebris - delta,
      0,
