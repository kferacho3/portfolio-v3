...
export const generateLevelSpawns = (preset: SpinBlockBoardPreset): LevelSpawns => {
  const halfBox = preset.boxSize / 2;
  const safe = Math.max(1, halfBox - 1.4);

  const coins: [number, number, number][] = Array.from({ length: preset.coinCount }, () => [
    randomInRange(safe * 1.6),
    0.5,
    randomInRange(safe * 1.6),
  ]);

  const gems: [number, number, number][] = Array.from({ length: preset.gemCount }, () => [
    randomInRange(safe * 1.6),
    0.5,
    randomInRange(safe * 1.6),
  ]);
