...
export function load() {
  try {
    ...
    if (typeof data?.selectedBall === 'number') state.selectedBall = data.selectedBall
    if (Array.isArray(data?.unlocked)) state.unlocked = data.unlocked
    if (Array.isArray(data?.unlockedBallIds)) {
      state.unlockedBallIds = data.unlockedBallIds.filter((id: number) => typeof id === 'number')
    } else if (Array.isArray(data?.unlocked)) {
      // Migrate from old unlocked array format
      state.unlockedBallIds = data.unlocked.map((unlocked: boolean, idx: number) => unlocked ? idx : -1).filter((id: number) => id >= 0)
    }
    if (typeof data?.showCredits === 'boolean') state.showCredits = data.showCredits
  } catch {
    // ignore
  }
}
...
export function save() {
  try {
      ...
      JSON.stringify({
        ...
        unlockedBallIds: Array.isArray(state.unlockedBallIds) ? state.unlockedBallIds : [0],
