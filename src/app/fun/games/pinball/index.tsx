  ...
  return (
    <Html fullscreen style={{ pointerEvents: 'none' }}>
      <div className="absolute top-4 left-4 bg-slate-900/80 backdrop-blur-sm px-4 py-3 rounded-xl border border-white/10 text-white">
        <div className="text-lg">Score: <strong className="text-cyan-400">{score}</strong></div>
        <div className="text-sm text-white/60">Best: <strong className="text-yellow-400">{bestScore}</strong></div>
      </div>
...
function BallAndCollisions({ args = [1.2, 32, 32], v = new THREE.Vector3() }) {
  ...
  return (
    <>
      ...
      {showSparks && <Sparks position={ballPositionRef.current} count={20} lifetime={150} />}
      {showScore && (
...
function Flipper({ side, controlsRef }: { side: FlipperSide; controlsRef: React.MutableRefObject<{ left: boolean; right: boolean }> }) {
  ...
  const [ref, api] = useBox(() => ({
    ...
    position: [side === 'left' ? -FLIPPER_CENTER_X : FLIPPER_CENTER_X, FLIPPER_Y, 0], // Z = 0
  ...
  useFrame((_, delta) => {
    ...
    const target = active ? (side === 'left' ? 1.15 : -1.15) : (side === 'left' ? -0.35 : 0.35);
...
const Block = forwardRef(({ blockId, shake = 0, args = [1, 1.5, 4], vec = new THREE.Vector3(), ...props }, ref) => {
  ...
  return (
    <group ref={group}>
      <RoundedBox ref={block} args={args} radius={0.4} smoothness={10} castShadow>
        <meshPhysicalMaterial 
          transmission={0.95} 
          roughness={0} 
          thickness={3} 
...
const Cube = forwardRef(({ blockId, shake = 0, args = [1, 1, 1], vec = new THREE.Vector3(), ...props }, ref) => {
  const group = useRef();
  const { increaseScore } = useContext(ScoreContext);
  const shakeRef = useRef(shake);
