...
export const PolarityHUD: React.FC = () => {
  ...
  const eventLabel =
    snap.event === 'PolarityStorm' ? 'Polarity Storm' : snap.event === 'Superconductor' ? 'Superconductor Zone' : snap.event === 'IonBloom' ? 'Ion Bloom' : '';
  useEffect(() => {
    ...
    <Html fullscreen style={{ pointerEvents: 'none' }}>
      <ArcadeHudShell gameId="polarity" className="absolute top-4 left-4 pointer-events-auto">
        <ArcadeHudCard className="min-w-[260px]">
          <div className="text-[11px] uppercase tracking-[0.32em] text-white/60">Magnetic Resonance</div>
          <div className="text-[11px] text-white/45">Flip → Whip → Stabilize</div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            {snap.event && <ArcadeHudPill label={`${eventLabel} ${Math.ceil(snap.eventTime)}s`} tone="accent" />}
          </div>
