...
import { PLATFORM_SPACING, PLATFORM_VISIBLE_ABOVE, PLATFORM_VISIBLE_BELOW, LEVER_SIZE, PALETTES } from '../constants';
...
export default function Levers({ pattern }: { pattern: PlatformPattern }) {
  ...
  useFrame(() => {
    ...
    for (const lever of pattern.levers) {
      ...
      dummy.scale.set(LEVER_SIZE, LEVER_SIZE * (isActivated ? 0.8 : 1), LEVER_SIZE);
      dummy.updateMatrix();
  ...
  return (
    <instancedMesh ref={leverRef} args={[undefined, leverMat, maxLevers]} frustumCulled={false}>
