...
import { consumeFixedStep, createFixedStepState, resetFixedStepState } from './engine/fixedStep';
...
const RING_SPACING = 0.84;
const VISIBLE_RING_COUNT = 210;
...
const platformBlend = (platform: OctaPlatformType, phase: number, time: number) => {
  if (platform === 'phase_lane') {
    return 0.5 + 0.5 * Math.sin(time * 1.95 + phase * 1.2);
  }
  if (platform === 'crusher_lane') {
    return 0.5 + 0.5 * Math.sin(time * 2.4 + phase * 0.9);
  }
  if (platform === 'resin_lane') {
    return 0.45 + 0.55 * (0.5 + 0.5 * Math.sin(time * 0.85 + phase));
  }
  if (platform === 'pulse_pad') {
    return 0.5 + 0.5 * Math.sin(time * 3.2 + phase * 1.4);
  }
  if (platform === 'overdrive_strip') {
    return 0.5 + 0.5 * Math.sin(time * 2.9 + phase * 0.7);
  }
  if (platform === 'split_rail') {
    return 0.5 + 0.5 * Math.sin(time * 2.15 + phase * 0.95);
  }
  if (platform === 'gravity_drift') {
    return 0.5 + 0.5 * Math.sin(time * 1.55 + phase * 1.7);
  }
  return 0;
};
...
const obstacleState = (meta: RingLaneMeta, time: number): ObstacleState => {
  ...
  if (meta.obstacle === 'arc_blade') {
    tunedClosed = clamp(
      closedBlend * 1.08 + Math.sin(time * 3.4 + meta.obstaclePhase) * 0.08,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend + Math.sin(time * 7.8 + meta.obstaclePhase * 2.1) * 0.18,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 0.86 +
        (0.5 + 0.5 * Math.sin(time * 2.2 + meta.obstaclePhase)) * 0.28,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 0.9 +
        (0.5 + 0.5 * Math.sin(time * 5.6 + meta.obstaclePhase * 2.0)) * 0.22,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 0.85 +
        (0.5 + 0.5 * Math.sin(time * 4.5 + meta.obstaclePhase * 1.2)) * 0.24,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 0.94 + Math.sin(time * 2.9 + meta.obstaclePhase) * 0.1,
      0,
      1
    );
    dangerThreshold = 0.4;
    ...
    tunedClosed = clamp(
      closedBlend * 0.9 +
        (0.5 + 0.5 * Math.sin(time * 1.8 + meta.obstaclePhase)) * 0.18,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 1.06 +
        (0.5 + 0.5 * Math.sin(time * 6.1 + meta.obstaclePhase * 1.4)) * 0.08,
      0,
      1
    );
    dangerThreshold = 0.36;
    ...
    tunedClosed = clamp(
      closedBlend * 0.84 +
        (0.5 + 0.5 * Math.sin(time * 8.8 + meta.obstaclePhase * 2.8)) * 0.25,
      0,
      1
    );
    dangerThreshold = 0.5;
    ...
    tunedClosed = clamp(
      closedBlend * 1.02 + Math.sin(time * 4.4 + meta.obstaclePhase * 1.7) * 0.12,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 1.05 + Math.sin(time * 5.9 + meta.obstaclePhase) * 0.14,
      0,
      1
    );
    dangerThreshold = 0.4;
    ...
    tunedClosed = clamp(
      closedBlend * 0.9 +
        (0.5 + 0.5 * Math.sin(time * 1.3 + meta.obstaclePhase)) * 0.3,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 0.88 + Math.sin(time * 6.7 + meta.obstaclePhase * 1.5) * 0.18,
      0,
      1
    );
    ...
    tunedClosed = clamp(
      closedBlend * 0.82 +
        (0.5 + 0.5 * Math.sin(time * 7.2 + meta.obstaclePhase * 2.4)) * 0.28,
      0,
      1
    );
...
const collectibleColor = (type: RingData['collectibleType']) => {
  if (type === 'core') return '#ffd5a1';
  if (type === 'sync') return '#a0f6ff';
  return '#8ae8ff';
};
...
const platformColor = (platform: OctaPlatformType) => {
  ...
  if (platform === 'split_rail' || platform === 'gravity_drift') return '#8ddcff';
  if (platform === 'pulse_pad' || platform === 'spring_pad') return '#ffc787';
  if (platform === 'warp_gate' || platform === 'phase_lane') return '#cb95ff';
  if (platform === 'resin_lane') return '#90e08f';
  if (platform === 'crusher_lane') return '#ff9696';
  return '#a8dbff';
};
...
const obstacleColor = (obstacle: OctaObstacleType) => {
  ...
  if (obstacle === 'flame_jet') return '#ff7344';
  if (obstacle === 'ember_wave') return '#ff9554';
  if (obstacle === 'ion_barrier') return '#8fe4ff';
  if (obstacle === 'pulse_laser') return '#ff87dc';
  return '#ff8e7e';
};
...
const runnerScaleFor = (shape: OctaRunnerShape) => {
  if (shape === 'cube') return 0.54;
  if (shape === 'tri_prism') return 0.62;
  if (shape === 'hex_prism') return 0.58;
  if (shape === 'pyramid') return 0.64;
  if (shape === 'tetra') return 0.7;
  if (shape === 'octa') return 0.62;
  if (shape === 'dodeca') return 0.58;
  if (shape === 'icosa') return 0.56;
  if (shape === 'star_prism') return 0.56;
  return 0.62;
};
...
export default function OctaSurge() {
  ...
  const simRef = useRef(
    createSimState(
      Math.floor(STAGE_PROFILES[0].sides / 2),
      STAGE_PROFILES[0].sides,
      STAGE_PROFILES[0].id
    )
  );
  const fixedStepRef = useRef(createFixedStepState(1 / 120, 0.05, 8));
  ...
  const geometries = useMemo(
    () => ({
      obstacleBlade: new THREE.BoxGeometry(0.84, 0.1, 0.26),
      obstacleGate: new THREE.TorusGeometry(0.32, 0.085, 10, 24),
      ...
      collectible: new THREE.OctahedronGeometry(0.25, 0),
      playerHalo: new THREE.TorusGeometry(0.62, 0.045, 12, 64),
      ghostCore: new THREE.IcosahedronGeometry(0.22, 1),
      ghostRing: new THREE.TorusGeometry(0.44, 0.034, 12, 40),
      runnerShapes: {
        cube: new THREE.BoxGeometry(1, 1, 1),
        tri_prism: new THREE.CylinderGeometry(0.62, 0.62, 1.1, 3),
        hex_prism: new THREE.CylinderGeometry(0.58, 0.58, 1.1, 6),
        pyramid: new THREE.ConeGeometry(0.7, 1.2, 4),
        tetra: new THREE.TetrahedronGeometry(0.78, 0),
        octa: new THREE.OctahedronGeometry(0.76, 0),
        dodeca: new THREE.DodecahedronGeometry(0.72, 0),
        icosa: new THREE.IcosahedronGeometry(0.7, 0),
        star_prism: new THREE.CylinderGeometry(0.52, 0.7, 1.2, 8, 1, false),
        fortress: new THREE.BoxGeometry(1.08, 0.82, 1.08),
      } as Record<OctaRunnerShape, THREE.BufferGeometry>,
    }),
    []
  );
  ...
  const glitchDelay = useMemo(() => new THREE.Vector2(1.4, 3.2), []);
  const glitchDuration = useMemo(() => new THREE.Vector2(0.08, 0.16), []);
  const glitchStrength = useMemo(() => new THREE.Vector2(0.02, 0.12), []);
  ...
  const initializeRun = useCallback(() => {
    ...
    ringGenRef.current.reset(
      queuedReplay?.seed ?? snap.worldSeed,
      queuedReplay?.mode ?? snap.mode
    );
    ...
    resetFixedStepState(fixedStepRef.current);
  ...
  }, [getRing, snap.cameraMode, snap.mode, snap.worldSeed]);
  ...
  const startRun = useCallback(() => {
    ...
    if (replayPlayback) {
      ...
      useOctaRuntimeStore.setState({
        mode: replayPlayback.mode,
        cameraMode: replayPlayback.cameraMode,
      });
    }
  ...
  const updateInstances = useCallback(() => {
    ...
    for (let r = 0; r < VISIBLE_RING_COUNT; r += 1) {
      ...
      const warp =
        Math.sin(ring.warpSeed + sim.runTime * 0.7 + ring.index * 0.11) *
        (0.012 + stage.id * 0.008);
      ...
      for (let lane = 0; lane < ring.sides; lane += 1) {
        ...
        if (meta.platform !== 'smooth_lane' && platformCount < platformCountMax) {
          ...
          const platformRadius = GAME.radius - 1.0 + platformPulse * 0.08;
          ...
          tempColorA
            ...
            .lerp(tempColorB.set(stageVisual.platform), 0.3)
            .multiplyScalar(0.58 + platformPulse * 0.58 + sim.audioReactive * 0.2);
        ...
        tempColorA
          ...
          .lerp(tempColorB.set(stageVisual.hazard), 0.32)
          .multiplyScalar(0.58 + hazard.closedBlend * 0.84 + sim.audioReactive * 0.2);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      scene.fog = null;
      geometries.obstacleBlade.dispose();
      geometries.obstacleGate.dispose();
      geometries.obstacleCore.dispose();
      geometries.obstacleSpire.dispose();
      geometries.platformPulse.dispose();
      geometries.collectible.dispose();
      geometries.playerHalo.dispose();
      geometries.ghostCore.dispose();
      geometries.ghostRing.dispose();
      Object.values(geometries.runnerShapes).forEach((geometry) => geometry.dispose());
    };
  }, [geometries, gl, scene]);
  ...
  useFrame((state, rawDelta) => {
    ...
    if (snap.phase !== 'playing') {
      ...
      if (playerRef.current) {
        ...
        playerRef.current.position.set(
          px,
          py,
          GAME.playerZ + Math.sin(state.clock.elapsedTime * 2.8) * 0.04
        );
      }
    ...
    const shapePrevTap = input.justPressed.has('z');
    const shapeNextTap = input.justPressed.has('x');
    ...
    if (shapePrevTap) octaSurgeState.cycleRunnerShape(-1);
    if (shapeNextTap) octaSurgeState.cycleRunnerShape(1);
    ...
    consumeFixedStep(fixedStepRef.current, frameDelta, (fixedDelta) => {
      const replay = replayRef.current;
      replay.frame += 1;
      ...
      if (sim.flipCooldown <= 0 && flipQueueRef.current > 0) {
        ...
        sim.laneIndex = normalizeLane(
          sim.laneIndex + Math.floor(sim.sides / 2),
          sim.sides
        );
        sim.flipPulse = 1;
      ...
      const reactiveTarget = clamp(
        0.2 +
          0.35 * (0.5 + 0.5 * Math.sin(sim.runTime * 2.8 + sim.speed * 0.65)) +
          sim.dangerPulse * 0.35,
        0,
        1
      );
      ...
      sim.speed = THREE.MathUtils.lerp(
        sim.speed,
        targetSpeed,
        1 - Math.exp(-fixedDelta * 2.6)
      );
      ...
      sim.laneFloat = positiveMod(
        sim.laneFloat + sim.laneVelocity * fixedDelta,
        sim.sides
      );
      ...
      while (sim.lastCrossedRing < baseRing && !sim.ended) {
        ...
        if (ring.sides !== sim.sides) {
          ...
          const remapped = normalizeLane(
            Math.round(remapAngle / laneStep(ring.sides)),
            ring.sides
          );
        ...
        const platformGate = platformBlend(
          meta.platform,
          meta.platformPhase,
          sim.runTime
        );
        ...
        const nearHazard =
          (leftMeta &&
            leftMeta.obstacle !== 'none' &&
            obstacleState(leftMeta, sim.runTime).danger) ||
          (rightMeta &&
            rightMeta.obstacle !== 'none' &&
            obstacleState(rightMeta, sim.runTime).danger);
    ...
    if (playerRef.current) {
      ...
      playerRef.current.scale.setScalar(pulse * runnerScaleFor(snap.runnerShape));
    ...
    if (cameraMode === 'firstPerson') {
      ...
      cam.position.y = THREE.MathUtils.lerp(
        ...
        Math.sin(GAME.playerAngle) * (GAME.radius - 0.58) +
          Math.cos(sim.runTime * 10.2) * shake,
        1 - Math.exp(-frameDelta * 8)
      );
      cam.position.z = THREE.MathUtils.lerp(
        cam.position.z,
      ...
      cam.fov = THREE.MathUtils.lerp(
        cam.fov,
        78 + speedRatio * 4 + sim.flipPulse * 2.2,
        1 - Math.exp(-frameDelta * 7)
      );
    } else if (cameraMode === 'topDown') {
      cam.position.x = THREE.MathUtils.lerp(
        cam.position.x,
        Math.sin(sim.runTime * 4.2) * shake * 0.3,
      ...
      cam.position.y = THREE.MathUtils.lerp(
        cam.position.y,
        -0.36 +
          Math.cos(sim.rotation * 0.52) * 0.05 +
          Math.cos(sim.runTime * 7.2) * shake * 0.7,
        1 - Math.exp(-frameDelta * 6)
      );
      cam.position.z = THREE.MathUtils.lerp(
        cam.position.z,
      ...
      cam.fov = THREE.MathUtils.lerp(
        cam.fov,
        60 + speedRatio * 4 + sim.flipPulse * 1.8,
        1 - Math.exp(-frameDelta * 6)
      );
    }
    ...
    const fxScale =
      snap.fxLevel === 'full' ? 1 : snap.fxLevel === 'medium' ? 0.76 : 0.54;
    if (bloomRef.current) {
      const targetBloom =
        (0.42 +
          sim.audioReactive * 0.72 +
          speedRatio * 0.28 +
          sim.flipPulse * 0.26 +
          sim.stageFlash * 0.2) *
        fxScale;
  ...
  const runnerGeometry =
    geometries.runnerShapes[snap.runnerShape] ?? geometries.runnerShapes.cube;
  ...
  return (
    <group>
      ...
      <group ref={worldRef}>
        ...
        <instancedMesh ref={platformMeshRef} args={[undefined, undefined, platformCountMax]}>
          <primitive object={geometries.platformPulse} attach="geometry" />
          <meshStandardMaterial
            vertexColors
            metalness={0.32}
            roughness={0.1}
            emissive={FIBER_COLORS.platform}
            emissiveIntensity={0.42}
          />
        </instancedMesh>

        <instancedMesh ref={obstacleBladeMeshRef} args={[undefined, undefined, obstacleCountMax]}>
          <primitive object={geometries.obstacleBlade} attach="geometry" />
          <meshStandardMaterial
            vertexColors
            metalness={0.62}
            roughness={0.1}
        ...
        <instancedMesh ref={obstacleGateMeshRef} args={[undefined, undefined, obstacleCountMax]}>
          <primitive object={geometries.obstacleGate} attach="geometry" />
          <meshStandardMaterial
            vertexColors
            metalness={0.42}
            roughness={0.12}
        ...
        <instancedMesh ref={obstacleCoreMeshRef} args={[undefined, undefined, obstacleCountMax]}>
          <primitive object={geometries.obstacleCore} attach="geometry" />
          <meshStandardMaterial
            vertexColors
            metalness={0.22}
            roughness={0.06}
        ...
        <instancedMesh ref={obstacleSpireMeshRef} args={[undefined, undefined, obstacleCountMax]}>
          <primitive object={geometries.obstacleSpire} attach="geometry" />
          <meshStandardMaterial
            vertexColors
            metalness={0.54}
            roughness={0.14}
        ...
        <instancedMesh ref={collectibleMeshRef} args={[undefined, undefined, collectibleCountMax]}>
          <primitive object={geometries.collectible} attach="geometry" />
          <meshStandardMaterial
            vertexColors
            metalness={0.35}
            roughness={0.06}
            emissive={FIBER_COLORS.collectible}
      ...
      <group ref={playerRef}>
        <mesh geometry={runnerGeometry} rotation={[Math.PI / 2, 0, 0]}>
          <meshStandardMaterial
        ...
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <primitive object={geometries.playerHalo} attach="geometry" />
          <meshBasicMaterial
      ...
      <group ref={ghostRef} visible={false}>
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <primitive object={geometries.ghostCore} attach="geometry" />
          <meshBasicMaterial
            color="#8beeff"
            transparent
            opacity={0.44}
        ...
        <mesh rotation={[Math.PI / 2, 0, 0]}>
          <primitive object={geometries.ghostRing} attach="geometry" />
          <meshBasicMaterial
            color="#d4f7ff"
            transparent
            opacity={0.5}
