    ...
    return new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main(){
          ...
          float rings = sin((length(vPos.xz)*0.02) + uTime*0.9) * 0.08;
          float stars = step(0.989, hash(floor(vPos.xz*0.28))) * 0.85;
          vec3 col = mix(uB, uA, h + rings);
...
const LowPolyGroundVisual: React.FC = () => {
  const geom = useMemo(() => {
...
const ScenePostFX: React.FC<{ boost: number }> = ({ boost }) => {
  const strength = clamp(0.6 + boost * 1.0, 0.6, 1.8);
  return (
    <EffectComposer multisampling={0}>
      <Bloom intensity={strength} luminanceThreshold={0.2} mipmapBlur />
      <ChromaticAberration
        offset={new THREE.Vector2(0.0009, 0.00075)}
...
const Ground: React.FC = () => {
  ...
  return (
    <mesh ref={ref} receiveShadow>
      ...
      {/* physics collider; visual ground is separate */}
      <meshStandardMaterial color="#070b12" transparent opacity={0} />
    </mesh>
  );
};
