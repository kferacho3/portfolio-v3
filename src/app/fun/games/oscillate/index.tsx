...
type RunStatus = {
  ...
  t: number;
  seg: number;
  s: number; // signed distance along current segment (-lenNeg..lenPos)
  dir: -1 | 1;
  y: number;
  vy: number;
  alive: boolean;
          ...
          <div className="pointer-events-auto w-[min(560px,92vw)] rounded-3xl bg-white/90 p-6 shadow-xl backdrop-blur">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-black/60">
                    Levels are permanent + deterministic.
                  </div>
                </div>
            ...
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
              <div className="text-xs text-black/40 leading-relaxed">
                ...
                Ketchapp × mauigo games (Marius Gerlich).
              </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="pointer-events-auto w-[min(720px,94vw)] rounded-3xl bg-white/95 p-6 shadow-xl backdrop-blur">
            ...
            <div className="mt-4 text-xs text-black/40">
              Tip: You earn gems by clearing levels. (Ad rewards not wired here
...
function Scene() {
  ...
  const mats = React.useMemo(() => {
    ...
    const wallDark = new THREE.MeshStandardMaterial({
      color: new THREE.Color(COLORS.wallDark),
      roughness: 0.72,
      metalness: 0.05,
    });
    ...
    const pillar = new THREE.MeshStandardMaterial({
      ...
      roughness: 0.25,
      metalness: 0.0,
      emissive: new THREE.Color('#2ad8ff'),
  ...
  React.useEffect(() => {
    ...
    if (snap.phase === 'menu' || snap.phase === 'playing') {
      levelRef.current = onePathState.buildLevel(
        onePathState.level,
        onePathState.mode
      );
  ...
  function getSeg(lvl: OnePathLevel, segIndex: number) {
    ...
    const oz = seg.origin.z;
    const ax = ox - dir.x * seg.lenNeg;
    const az = oz - dir.z * seg.lenNeg;
    const bx = ox + dir.x * seg.lenPos;
    const bz = oz + dir.z * seg.lenPos;
    const len = seg.lenNeg + seg.lenPos;
    return { seg, dir, ox, oz, ax, az, bx, bz, len };
  }

  function getDefaultDir(seg: OnePathLevel['segments'][number]) {
    return seg.lenPos >= seg.lenNeg ? 1 : -1;
  }

  function getGoalPos(lvl: OnePathLevel) {
    const goalSeg = lvl.segments[lvl.goal.segIndex];
    if (!goalSeg) return { x: 0, z: 0 };
    return {
      x: goalSeg.origin.x + goalSeg.dir.x * lvl.goal.offset,
      z: goalSeg.origin.z + goalSeg.dir.z * lvl.goal.offset,
    };
  }
  ...
  function attemptTurn(lvl: OnePathLevel) {
    ...
    const exitX = segData.ox + segData.dir.x * seg.exit;
    const exitZ = segData.oz + segData.dir.z * seg.exit;
    const distToExit = Math.hypot(exitX - px, exitZ - pz);

    const turnWindow = lvl.turnRadius;
    const snapWindow = lvl.snapRadius;

    if (distToExit <= turnWindow) {
      // perfect feedback scales with how tight your timing was
      r.perfectFlash = Math.max(r.perfectFlash, 1 - distToExit / turnWindow);
  ...
  function CameraRig() {
    ...
    useFrame((_, dt) => {
      ...
      const seg = getSeg(lvl, clamp(r.seg, 0, lvl.segments.length - 1));
      const px = seg.ox + seg.dir.x * r.s;
      const pz = seg.oz + seg.dir.z * r.s;
  ...
  useFrame((_, dt) => {
    ...
    if (snap.phase !== 'playing') {
      ...
      const goal = getGoalPos(lvl);
      if (portalRef.current) {
        portalRef.current.position.set(
          goal.x,
          CONST.BASE_H + CONST.DECK_H + 0.18,
          goal.z
        );
    ...
    if (r.alive && !r.cleared) {
      ...
      const seg = segData.seg;
      const prevS = r.s;
      const nextS = r.s + r.dir * lvl.speed * dt;
      r.s = nextS;

      // goal reached?
      if (r.seg === lvl.goal.segIndex) {
        const goalRadius = lvl.turnRadius * 1.15;
        const crossed =
          (prevS - lvl.goal.offset) * (nextS - lvl.goal.offset) <= 0;
  ...
  const segmentMeshes = lvl.segments.map((seg, idx) => {
    ...
    const bz = seg.origin.z + seg.dir.z * seg.lenPos;
    const dx = bx - ax;
    const dz = bz - az;
    ...
    const deckScale = new THREE.Vector3(
      len,
      CONST.DECK_H,
      lvl.bridgeWidth * 0.92
    );
  ...
  function NextSegmentHighlight() {
    ...
    useFrame(() => {
      ...
      if (nextSegIndex < 0 || nextSegIndex >= lvl.segments.length) {
        ref.current.visible = false;
        return;
      }

      const seg = lvl.segments[nextSegIndex];
      const ax = seg.origin.x - seg.dir.x * seg.lenNeg;
      const az = seg.origin.z - seg.dir.z * seg.lenNeg;
      const bx = seg.origin.x + seg.dir.x * seg.lenPos;
      const bz = seg.origin.z + seg.dir.z * seg.lenPos;
      ...
      <group ref={groupRef}>
        ...
        {lvl.segments.map((seg, segIdx) => {
          ...
          const renderWall = (
            ...
            const hp01 = wall.unbreakable
              ? 1
              : clamp(wall.hp / wall.maxHp, 0, 1);
            const scaleY = CONST.WALL_H * (0.7 + hp01 * 0.3);
            const y = CONST.BASE_H + CONST.DECK_H + scaleY * 0.5;
            return (
              <group key={key} position={[x, y, z]} rotation={[0, -angle, 0]}>
                <mesh
                  geometry={geoms.unitBox}
                  material={mats.pillar}
                  scale={[CONST.WALL_T, scaleY, wallWidth]}
                  visible={!wall.broken}
                  castShadow
                />
              </group>
          ...
          return (
            <group key={`${seg.id}_walls`} visible={visible}>
              {renderWall(
                ...
                seg.origin.z - seg.dir.z * seg.lenNeg,
                `${seg.id}_neg`
              )}
              {renderWall(
                seg.walls.pos,
                seg.origin.x + seg.dir.x * seg.lenPos,
                seg.origin.z + seg.dir.z * seg.lenPos,
                `${seg.id}_pos`
              )}
            </group>
          );
        ...
        {lvl.gems.map((g) => (
          <mesh
            ...
            rotation={[
              0.4,
              (parseInt(g.id.split('_').pop() || '0', 10) * 0.9) %
                (Math.PI * 2),
              0,
            ]}
            visible={!g.collected}
...
function PerfectFlash({ run }: { run: React.MutableRefObject<RunStatus> }) {
  ...
  useFrame(() => {
    ...
    const p = ref.current.parent.getObjectByName('__ball') as
      | THREE.Object3D
      | undefined;
    void p;
  ...
  useFrame((state) => {
    ...
    ref.current.lookAt(
      state.camera.position
        .clone()
        .add(state.camera.getWorldDirection(new THREE.Vector3()))
    );
