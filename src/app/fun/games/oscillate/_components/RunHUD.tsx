...
export const RunHUD: React.FC<{ run: React.MutableRefObject<RunStatus> }> = ({
  run,
}) => {
    ...
    <div className="absolute top-6 right-6 pointer-events-none">
      <div className="rounded-2xl bg-black/5 px-4 py-3 text-black/70 shadow-sm">
        ...
        <div className="mt-1 flex items-center gap-2">
          <div
            className={`text-xs font-semibold ${comboOn ? 'text-black/80' : 'text-black/40'}`}
          >
          ...
          {comboOn && (
            <div className="h-2 w-24 overflow-hidden rounded-full bg-black/10">
              <div
                ...
                style={{
                  width: `${Math.min(100, (ui.comboTime / 2.6) * 100)}%`,
                }}
              />
            </div>
          )}
        ...
        <div className="mt-2 flex items-center justify-between gap-3 text-xs">
          <div
            className={`font-semibold ${syncReady ? 'text-emerald-600' : 'text-black/50'}`}
          >
            SYNC
          </div>
          <div className="h-2 w-24 overflow-hidden rounded-full bg-black/10">
            <div
              ...
              style={{
                width: `${syncReady ? 100 : Math.max(0, 100 - (ui.pulse / run.current.pulseCdMax) * 100)}%`,
              }}
            />
          </div>
        </div>
