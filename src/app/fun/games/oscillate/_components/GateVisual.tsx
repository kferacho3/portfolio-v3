...
export const GateVisual: React.FC<{
  ...
  useFrame((_, dt) => {
    ...
    if (seg.axis === 'x') {
      ...
      if (leftRef.current) {
        ...
        leftRef.current.position.set(endX + offsetAlong, deckY, seg.z + (-seg.halfWidth + openStart) * 0.5);
        leftRef.current.scale.set(thickness, CONST.DECK_H * 1.05, leftLen);
      }
      if (rightRef.current) {
        rightRef.current.visible = rightLen > 0.01;
        rightRef.current.position.set(endX + offsetAlong, deckY, seg.z + (openEnd + seg.halfWidth) * 0.5);
        rightRef.current.scale.set(thickness, CONST.DECK_H * 1.05, rightLen);
      }
      if (glowRef.current) {
        glowRef.current.position.set(endX + seg.dir * 0.22, deckY + 0.01, seg.z + seg.bridgeOffset);
        glowRef.current.scale.set(1, 1, openW);
        (glowRef.current.material as THREE.MeshStandardMaterial).opacity = 0.55 + Math.sin(t.current * 5.2) * 0.15;
      }
    } else {
      const endZ = seg.z + seg.dir * seg.length;
      if (leftRef.current) {
        leftRef.current.visible = leftLen > 0.01;
        leftRef.current.position.set(seg.x + (-seg.halfWidth + openStart) * 0.5, deckY, endZ + offsetAlong);
        leftRef.current.scale.set(leftLen, CONST.DECK_H * 1.05, thickness);
      }
      if (rightRef.current) {
        rightRef.current.visible = rightLen > 0.01;
        rightRef.current.position.set(seg.x + (openEnd + seg.halfWidth) * 0.5, deckY, endZ + offsetAlong);
        rightRef.current.scale.set(rightLen, CONST.DECK_H * 1.05, thickness);
      }
      if (glowRef.current) {
        glowRef.current.position.set(seg.x + seg.bridgeOffset, deckY + 0.01, endZ + seg.dir * 0.22);
  ...
  return (
    <group>
      <mesh ref={leftRef} geometry={geoms.unitBox} material={mats.wall as any} />
      <mesh ref={rightRef} geometry={geoms.unitBox} material={mats.wall as any} />
