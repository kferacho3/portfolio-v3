          ...
          <div className="pointer-events-auto w-[min(600px,92vw)] rounded-3xl bg-white/90 p-6 shadow-xl backdrop-blur">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <div className="mt-2 text-black/70">
                  Bounce between side walls. Tap <span className="font-semibold text-black/80">near the end</span> to
                  turn onto the next bridge.
                </div>
              </div>
              <button
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-black/5 p-4">
                <div className="text-sm font-semibold text-black/80">Timing window</div>
                <div className="mt-1 text-sm text-black/60">
                  The bridge has an offset. Tap when the ball lines up with the glowing gate.
                </div>
              </div>
              <div className="rounded-2xl bg-black/5 p-4">
                <div className="text-sm font-semibold text-black/80">Save tool</div>
                <div className="mt-1 text-sm text-black/60">
                  Once in a while, a "SYNC" save will rescue a near-miss (cooldown).
                </div>
              </div>
            </div>
            ...
            <div className="mt-5 rounded-2xl bg-black/5 p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-sm text-black/60">Levels are deterministic.</div>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    className="h-10 w-10 rounded-xl bg-white/80 text-black font-black hover:bg-white"
                    onClick={() => oscillateState.selectLevel(Math.max(1, snap.selectedLevel - 1))}
                  >
                    –
                  </button>
                  ...
                  <button
                    className="h-10 w-10 rounded-xl bg-white/80 text-black font-black hover:bg-white"
                    onClick={() => oscillateState.selectLevel(snap.selectedLevel + 1)}
                  >
                    +
                  </button>
                </div>
            ...
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
              <div className="text-xs text-black/40 leading-relaxed">
                Inspired by <span className="font-semibold text-black/50">The Walls</span> — Ketchapp.
              </div>
      ...
      {snap.phase === 'shop' && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="pointer-events-auto w-[min(720px,94vw)] rounded-3xl bg-white/95 p-6 shadow-xl backdrop-blur">
            ...
            <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
              {snap.skins.map((s) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full border border-black/10" style={{ background: s.color }} />
            ...
            <div className="mt-4 text-xs text-black/40">Tip: Clear levels to earn gems.</div>
          </div>
        </div>
      )}
      ...
      {snap.phase === 'gameover' && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="pointer-events-auto w-[min(560px,92vw)] rounded-3xl bg-white/90 p-6 shadow-xl backdrop-blur">
            ...
            <div className="mt-2 text-black/70">Tap near the end when the ball lines up with the gate.</div>
