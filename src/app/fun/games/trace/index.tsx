...
function TraceScene() {
  ...
  useFrame((state, delta) => {
    ...
    if (fillMeshRef.current) {
      ...
      for (let i = 0; i < total && idx < FILL_CELL_CAP; i += 1) {
        ...
        const pulse = 0.9 + Math.sin((runtime.elapsed - cell.createdAt) * 5.2) * 0.05;

        dummy.position.set(cell.x, 0.028, cell.z);
        dummy.scale.set(CELL_SIZE * 0.86, 0.035, CELL_SIZE * 0.86);
        dummy.rotation.set(0, 0, 0);
        dummy.updateMatrix();
        fillMeshRef.current.setMatrixAt(idx, dummy.matrix);

        segColor.copy(themeTrailA).lerp(themeTrailB, gradient).multiplyScalar(pulse);
  ...
  return (
    <>
      ...
      <instancedMesh ref={fillMeshRef} args={[undefined, undefined, FILL_CELL_CAP]}>
        <boxGeometry args={[1, 1, 1]} />
        <meshBasicMaterial vertexColors transparent opacity={0.34} toneMapped={false} />
      </instancedMesh>
