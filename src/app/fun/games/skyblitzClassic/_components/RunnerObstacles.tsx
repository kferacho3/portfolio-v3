...
import { generateInitialObstacles, generateRandomPosition } from '../utils/spawn';
  ...
  return (
    <>
      {obstacles.map((obstacle, index) => (
        <RunnerObstacle key={`runner-ob-${index}`} position={obstacle.position} scale={obstacle.scale} />
      ))}
    </>
  );
};
