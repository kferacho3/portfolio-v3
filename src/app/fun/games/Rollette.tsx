...
const ScoreDisplay = ({ children }) => (
  <div style={{ fontSize: 24, fontWeight: 700, color: '#FFD700' }}>
    {children}
  </div>
);

const HealthDisplay = ({ health, children }) => {
  const color = health > 66 ? '#00FF00' : health > 33 ? '#FFFF00' : '#FF0000';
  return (
    <div style={{ fontSize: 24, fontWeight: 700, color }}>{children}</div>
  );
};
const types = {
  obstacle: {
