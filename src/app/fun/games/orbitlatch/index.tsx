...
function getHazard(baseSeed: number, index: number, out: { pos: THREE.Vector3; r: number }) {
...
export default function OrbitLatch() {
  ...
  const [seed, setSeed] = useState<number>(() => (Date.now() ^ (Math.random() * 1e9)) >>> 0)
  ...
  return (
    <div className="relative w-full h-full select-none" style={{ background: 'linear-gradient(to bottom, #ffb3d9 0%, #a8c8ff 100%)' }}>
      <Canvas
        dpr={[1, 2]}
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-[min(520px,92vw)] rounded-xl bg-black/55 border border-white/10 p-6 text-white backdrop-blur-sm">
            ...
            <div className="mt-2 text-sm text-white/80 leading-relaxed">
              Orbit around pylons. Tap to launch on a tangent and <span className="font-semibold">latch</span> the next pylon.
              Miss a latch or hit a hazard sphere and it’s over.
            </div>
            ...
            <div className="mt-4 text-xs text-white/60">
              Tip: launch when you’re on the forward arc so your tangent points toward the next pylon.
            </div>
          </div>
        </div>
      )}
...
function OrbitLatchScene({
  ...
  const pylonGeo = useMemo(() => new THREE.CylinderGeometry(0.28, 0.34, 0.9, 16, 1), [])
  const pylonTopGeo = useMemo(() => new THREE.CylinderGeometry(0.32, 0.32, 0.12, 16, 1), [])
  const hazardGeo = useMemo(() => new THREE.IcosahedronGeometry(0.24, 0), [])
  const skyGeo = useMemo(() => new THREE.SphereGeometry(200, 32, 32), [])
  const skyMat = useMemo(() => new THREE.MeshBasicMaterial({ color: '#a8c8ff', side: THREE.BackSide, depthWrite: false }), [])

  const floorMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#b8d4f0', roughness: 0.7, metalness: 0.1 }), [])
  const pylonMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#8b5cf6', roughness: 0.4, metalness: 0.15, emissive: new THREE.Color('#7c3aed'), emissiveIntensity: 0.2 }), [])
  const pylonTopMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#facc15', roughness: 0.2, metalness: 0.3, emissive: new THREE.Color('#facc15'), emissiveIntensity: 0.8 }), [])
  const hazardMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#ff3a5e', roughness: 0.2, metalness: 0.1, emissive: new THREE.Color('#ff3a5e'), emissiveIntensity: 0.6 }), [])
  const ballMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#ef4444', roughness: 0.2, metalness: 0.3, emissive: new THREE.Color('#ef4444'), emissiveIntensity: 0.15 }), [])
  const lineMat = useMemo(() => new THREE.LineBasicMaterial({ color: '#ffffff', transparent: true, opacity: 0.6 }), [])
  ...
  return (
    <group position={[0, 0, -2]}>
      ...
      {/* Bright floor */}
      <mesh geometry={floorGeo} material={floorMat} rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.01, 60]} receiveShadow />
      ...
      {visibleAnchors.map((i) => {
        ...
        return (
          <group key={`a-${i}`} position={[tmpA.x, 0, tmpA.z]}>
            {/* Pylon body - purple */}
            <mesh geometry={pylonGeo} material={pylonMat} position={[0, 0.45, 0]} castShadow receiveShadow />
            {/* Glowing yellow top */}
            <mesh geometry={pylonTopGeo} material={pylonTopMat} position={[0, 0.96, 0]} castShadow />
            ...
            {(() => {
              ...
              if (currentIdx >= 0 && currentIdx < visibleAnchors.length - 1) {
                ...
                if (nextIdx !== undefined) {
                  ...
                  const points = [new THREE.Vector3(0, 0.96, 0), new THREE.Vector3(tmpB.x - tmpA.x, 0.96, tmpB.z - tmpA.z)]
                  const lineGeo = new THREE.BufferGeometry().setFromPoints(points)
                  return (
                    <line key={`line-${i}`} geometry={lineGeo} material={lineMat} />
                  )
                }
              }
              return null
...
function OrbitTrail({
  ...
  ORBIT_RADIUS: number
  BALL_Y: number
  getAnchorPos: (baseSeed: number, index: number, out: THREE.Vector3) => THREE.Vector3
}) {
