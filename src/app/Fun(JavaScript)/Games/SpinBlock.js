...
function isPositionOccupied(position) {
  ...
  return Object.values(gameState.blocks).some((blocksType) =>
    Object.values(blocksType).some((block) => {
      ...
      return (
        Math.round(blockX) === Math.round(position[0]) &&
        Math.round(blockY) === Math.round(position[1])
      );
    })
  );
}
      ...
      >
        ...
        <Physics gravity={[0, -80, 0]} timeStep="vary">
          ...
          <group {...bind()} rotation={rotation}>
            <Arena
              transmissiveMaterial={transmissiveMaterial}
              gameState={gameState}
            />
          </group>
...
function StationaryBlock({ id, position, color }) {
  return (
    <RigidBody
      ...
      onCollisionEnter={() => gameState.pong('stationary')}
    >
...
function BreakableBlock({ id, position, color }) {
  ...
  return (
    <RigidBody
      type="fixed"
      position={position}
      userData={{ type: 'breakable', id }} // Ensure userData is set for collision identification
...
function BouncyBlock({ id, position, color }) {
  return (
    <RigidBody
      ...
      onCollisionEnter={() => gameState.pong('bouncy')}
    >
      <CuboidCollider args={[1, 1, 1]} />
      <mesh>
...
function MovingBlock({ position }) {
  ...
  return (
    <RigidBody
      ...
      ref={ref}
      type="fixed"
      userData={{ type: 'moving' }}
      position={position}
      onCollisionEnter={() => gameState.pong('moving')}
    >
      <CuboidCollider args={[1, 1, 1]} />
      <mesh>
        <boxGeometry args={[1, 1, 1]} />
