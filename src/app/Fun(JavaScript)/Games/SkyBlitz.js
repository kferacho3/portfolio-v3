...
const Obstacles = ({ playerRef }) => {
  ...
  useFrame((state, delta) => {
    ...
    obstaclesRef.current.forEach((obstacle) => {
      if (
        playerRef.current.position.z - obstacle.position[2] >
        REPOSITION_THRESHOLD
      ) {
        // Reposition the obstacle far in front of the player
        const newPosition = generateRandomPosition(
          playerRef.current.position.z + OBSTACLE_SPREAD_Z
        );
...
const Player = ({ setScore, playerRef }) => {
  ...
  useFrame((state, delta) => {
    ...
    const direction = new THREE.Vector3(
      mouse.x / 5,
      Math.max(mouse.y, -0.1),
      0.5
    ).normalize(); // Z value adjusts the forward direction
    ...
    return Array.from({ length: NUM_OBSTACLES }, (_, i) => {
      return {
        ...
        position: [
          Math.random() * 6 - 3,
          0,
          PLAYER_START_Z - (i * OBSTACLE_SPREAD_Z) / NUM_OBSTACLES,
        ],
      };
    });
  };
    ...
    useFrame(() => {
      ...
      for (let i = 0; i < newObstacles.length; i++) {
        if (playerZ - newObstacles[i].position[2] > REPOSITION_THRESHOLD) {
          newObstacles[i].position = generateRandomPosition(
            playerZ + OBSTACLE_SPREAD_Z
          );
        }
      }
    ...
    <>
      <Canvas camera={{ position: [0, 0, 25], fov: 50 }}>
        ...
        <Physics gravity={[0, -30, 0]}>
          ...
          <Obstacles
            initialObstaclePositions={initialObstaclePositions}
            repositionObstacle={repositionObstacle}
            playerRef={playerRef}
            useObstaclePositions={useObstaclePositions}
          />
