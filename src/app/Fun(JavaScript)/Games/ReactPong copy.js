...
function Paddle({ setPaddlePosition, score, scoreColor }) {
  ...
  useFrame((state, delta) => {
    ...
    vec
      .set(state.pointer.x * 0.5, state.pointer.y * 0.85, 0.5)
      .unproject(camera);
...
function Arena() {
  ...
  // Walls setup
  const [bottomLeftWallRef] = useBox(() => ({
    position: [-arenaWidth / 4, -arenaHeight / 2, 0],
    args: [arenaWidth / 5, wallThickness, wallHeight],
    ...wallMaterial,
  }));
  const [bottomRightWallRef] = useBox(() => ({
    position: [arenaWidth / 4, -arenaHeight / 2, 0],
    args: [arenaWidth / 5, wallThickness, wallHeight],
    ...wallMaterial,
  }));
  const [leftWallRef] = useBox(() => ({
    ...
    args: [wallThickness, arenaHeight, wallHeight],
    ...wallMaterial,
  }));
  ...
  const [topWallRef] = useBox(() => ({
    position: [0, arenaHeight / 2 + wallThickness / 2, 0],
    args: [arenaWidth, wallThickness, wallHeight],
    ...wallMaterial,
  }));
      ...
      {[
        ...
        <mesh key={i} ref={ref}>
          <boxBufferGeometry
            ...
            args={
              i < 2
                ? [arenaWidth / 5, wallThickness, wallHeight]
                : [
                    i < 4 ? wallThickness : arenaWidth,
                    i < 4 ? arenaHeight : wallThickness,
                    wallHeight,
                  ]
            }
          />
