...
const types = {
  ...
  collectible: {
    green: {
      effect: (state) => (state.health = Math.min(state.health + 1, 100)),
      color: 'green',
      score: 10,
    },
    blue: {
      effect: (state) => (state.health = Math.min(state.health + 0.5, 100)),
      color: 'blue',
      score: 5,
    },
    white: {
      effect: (state) => (state.health = Math.min(state.health + 0.1, 100)),
      color: 'white',
      score: 2,
    },
...
function getDreiGeometryComponent(shape) {
  ...
  switch (shape) {
    ...
    default:
      return 'sphereBufferGeometry'; // Default to sphere if unknown
  }
}
...
function getDreiComponent(shape) {
  switch (shape) {
    ...
    case 'Box':
      return Box;
    default:
      return Sphere; // Default to Sphere if shape is unknown
  }
}
...
function getShapeArgs(shape) {
  switch (shape) {
    ...
    case 'Tetrahedron':
      return [0.05];
    case 'TorusKnot':
      return [0.05, 0.04, 64, 16];
    case 'Dodecahedron':
      return [0.05];
    case 'Box':
      return [0.05, 0.05, 0.05];
    default:
      return [0.1]; // Default args
  }
}
  ...
  const [ref, api] = useBox(() => ({
    ...
    onCollide: (e) => {
      ...
      if (e.body.userData.type === 'player') {
        // Assuming you set userData.type = 'player' on the player sphere
        console.log('Collision Detected with Player');
        const collisionPosition = [
          e.contact.bi.position.x,
          e.contact.bi.position.y,
          e.contact.bi.position.z,
        ];
...
const TorusKnotCollectible = ({
  ...
  return (
    <TorusKnot ref={ref} args={[0.5, 3, 100, 16]} position={position}>
...
const EnhancedPlayerSphere = ({ setGameState, playerPositionRef }) => {
  ...
  useFrame(() => {
    ...
    if (currentPosition.x < -halfGroundSize)
      api.position.set(-halfGroundSize, currentPosition.y, currentPosition.z);
    if (currentPosition.x > halfGroundSize)
      api.position.set(halfGroundSize, currentPosition.y, currentPosition.z);
    if (currentPosition.z < -halfGroundSize)
      api.position.set(currentPosition.x, currentPosition.y, -halfGroundSize);
    if (currentPosition.z > halfGroundSize)
      api.position.set(currentPosition.x, currentPosition.y, halfGroundSize);
  });
...
function generateItems(type, count) {
  ...
  for (let i = 0; i < count; i++) {
    const color =
      type === 'obstacle'
        ? ['purple', 'yellow', 'orange', 'red'][i % 4]
        : ['green', 'blue', 'white', 'gold'][i % 4];
    items.push({
      id: i,
      position: generatePosition(),
      type: type,
...
const Rollette = () => {
  ...
  const addItem = (type) => {
    const newItem = {
      ...
      variant:
        type === 'obstacle'
          ? ['purple', 'yellow', 'orange', 'red'][Math.floor(Math.random() * 4)]
          : ['green', 'blue', 'white', 'gold'][Math.floor(Math.random() * 4)],
    };
    setGameState((prevState) => ({
      ...prevState,
      [type === 'obstacle' ? 'obstacles' : 'collectibles']: [
        ...prevState[type === 'obstacle' ? 'obstacles' : 'collectibles'],
        newItem,
      ],
    }));
  };
  ...
  const handleCollisionWithEffect = (effectType, objectColor) => {
    ...
    setEffects((currentEffects) => [
      ...currentEffects,
      { ...newEffect, particles: newParticles },
    ]);
  ...
  const spawnItems = () => {
    setGameState((prevState) => ({
      ...
      obstacles: [
        ...prevState.obstacles,
        { id: Math.random(), position: generatePosition(), type: 'obstacle' },
      ],
      collectibles: [
        ...prevState.collectibles,
        {
          id: Math.random(),
          position: generatePosition(),
          type: 'collectible',
          ...
          <Physics iterations={5} gravity={[0, -20, 0]}>
            ...
            {barrierPositions.map((position, index) => (
              <Barrier
                key={index}
                position={position}
                dimensions={barrierDimensions[index < 2 ? 0 : 1]}
              />
            ))}
