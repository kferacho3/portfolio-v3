...
const bg =
  'https://racho-devs.s3.us-east-2.amazonaws.com/fun/gameAssets/bg.png';
  ...
  const increaseScore = (blockId, isCube = false) => {
    setBlockHits((prevHits) => {
      ...
      const pointsToAdd = isCube
        ...
        : 10 * Math.pow(2, currentHits - 1); // 100x for cubes, normal for others
      setCurrentAddedScore(pointsToAdd);
...
function BallAndCollisions({ args = [1.2, 32, 32] }) {
  ...
  useFrame(() => {
    ...
    api.position.subscribe((position) => {
      ...
      cam.current.position.lerp(
        new THREE.Vector3(position[0], cameraPositionY, cameraPositionZ),
        0.1
      );
  ...
  return (
    <>
      ...
      />
      {showSparks && (
        <Sparks position={ballPositionRef.current} count={20} lifetime={300} />
      )}
      {showScore && (
...
const EffectSphere = forwardRef(
  (
    ...
    const { sphereColors, handleSphereCollision, increaseScore } =
