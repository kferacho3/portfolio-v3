      ...
      {shapes.map((shape, index) => (
        ...
        >
          {shape.type === 'dodecahedron' && (
            <dodecahedronGeometry args={[shape.size, 0]} />
          )}
          {shape.type === 'sphere' && (
            <sphereGeometry args={[shape.size, 32, 32]} />
          )}
          {shape.type === 'cone' && (
            <coneGeometry args={[shape.size, shape.size * 2, 32]} />
          )}
...
export default function SpinBlock({ ready }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      setStationaryBlocks((prev) => [
        ...prev,
        <StationaryBlock position={position} key={`stationary-${i}`} />,
      ]);
    }
    ...
    for (let i = 0; i < 2; i++) {
      const position = getRandomPosition();
      setMovingBlocks((prev) => [
        ...prev,
        <MovingBlock position={position} key={`moving-${i}`} />,
      ]);
    }
    ...
    for (let i = 0; i < 10; i++) {
      const position = getRandomPosition();
      setBreakableBlocks((prev) => [
        ...prev,
        <BreakableBlock position={position} key={`breakable-${i}`} />,
      ]);
    }
...
function getRandomPosition() {
  ...
  return [
    x - arenaWidth / 2 + blockSize / 2,
    0,
    z - arenaDepth / 2 + blockSize / 2,
  ]; // Center the block in the cell
}
...
function Arena() {
  ...
  const grid = Array(blocksX)
    .fill(null)
    .map(() => Array(blocksZ).fill(false));
  ...
  return (
    <>
      ...
      <RigidBody
        restitution={1.1}
        position={[
          -(bottomWallGap / 2 + bottomWallLength / 2),
          -arenaHeight / 2,
          0,
        ]}
        type="fixed"
